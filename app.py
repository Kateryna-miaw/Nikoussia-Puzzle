...
function drawPuzzleEdge(c,x0,y0,x1,y1,dir){
  ...
  const nx=-dy/len*dir,ny=dx/len*dir,tabH=len*TAB_SIZE*1.1,tabW=len*0.12;
  const e1x=x0+dx*.35,e1y=y0+dy*.35,e2x=x0+dx*.65,e2y=y0+dy*.65;
  const n1x=e1x+nx*(-tabH*.15),n1y=e1y+ny*(-tabH*.15),n2x=e2x+nx*(-tabH*.15),n2y=e2y+ny*(-tabH*.15);
  c.lineTo(e1x,e1y);c.lineTo(n1x,n1y);
  c.bezierCurveTo(n1x+nx*tabH*.8-dx/len*tabW*1.2,n1y+ny*tabH*.8-dy/len*tabW*1.2,n1x+nx*tabH*1.1-dx/len*tabW*.2,n1y+ny*tabH*1.1-dy/len*tabW*.2,x0+dx*.5+nx*tabH,y0+dy*.5+ny*tabH);
  c.bezierCurveTo(n2x+nx*tabH*1.1+dx/len*tabW*.2,n2y+ny*tabH*1.1+dy/len*tabW*.2,n2x+nx*tabH*.8+dx/len*tabW*1.2,n2y+ny*tabH*.8+dy/len*tabW*1.2,n2x,n2y);
  c.lineTo(e2x,e2y);c.lineTo(x1,y1);
}
function createPiecePath(c,row,col,px,py,cw,ch){
  c.beginPath();c.moveTo(px,py);
  drawPuzzleEdge(c,px,py,px+cw,py,        row>0      ?-hEdges[row-1][col]:0);
  drawPuzzleEdge(c,px+cw,py,px+cw,py+ch,  col<GRID-1 ? vEdges[row][col]  :0);
  drawPuzzleEdge(c,px+cw,py+ch,px,py+ch,  row<GRID-1 ?-hEdges[row][col]  :0);
  drawPuzzleEdge(c,px,py+ch,px,py,         col>0      ?-vEdges[row][col-1]:0);
  c.closePath();
}
...
function buildTray(){
  ...
  const thumbSize=48,padding=TAB_SIZE*thumbSize,totalSize=thumbSize+padding*2;
...
function drawPieceThumb(cvs,piece,size,pad){
  ...
  const sx=piece.col*(img.width/GRID),sy=piece.row*(img.height/GRID),sw=img.width/GRID,sh=img.height/GRID,m=TAB_SIZE*Math.max(sw,sh)*1.2;
  t.drawImage(img,sx-m,sy-m,sw+m*2,sh+m*2,pad-TAB_SIZE*size*1.2,pad-TAB_SIZE*size*1.2,size+TAB_SIZE*size*2.4,size+TAB_SIZE*size*2.4);
...
function drawPieceOnBoard(piece){
  const px=boardX+piece.col*cellW,py=boardY+piece.row*cellH,m=TAB_SIZE*cellW*1.3;
  ctx.save();ctx.translate(1.5,2);createPiecePath(ctx,piece.row,piece.col,px,py,cellW,cellH);ctx.fillStyle='rgba(50,20,60,0.12)';ctx.fill();ctx.restore();
  ctx.save();createPiecePath(ctx,piece.row,piece.col,px,py,cellW,cellH);ctx.clip();
  const sx=piece.col*(img.width/GRID),sy=piece.row*(img.height/GRID),sw=img.width/GRID,sh=img.height/GRID,sm=TAB_SIZE*Math.max(sw,sh)*1.3;
  ctx.drawImage(img,sx-sm,sy-sm,sw+sm*2,sh+sm*2,px-m,py-m,cellW+m*2,cellH+m*2);
  createPiecePath(ctx,piece.row,piece.col,px,py,cellW,cellH);
  const bevel=ctx.createLinearGradient(px,py,px+cellW,py+cellH);
  bevel.addColorStop(0,'rgba(255,255,255,0.18)');bevel.addColorStop(.35,'rgba(255,255,255,0.03)');bevel.addColorStop(.65,'transparent');bevel.addColorStop(1,'rgba(40,10,50,0.1)');
  ctx.fillStyle=bevel;ctx.fill();ctx.restore();
  ctx.save();createPiecePath(ctx,piece.row,piece.col,px,py,cellW,cellH);ctx.strokeStyle='rgba(184,160,214,0.12)';ctx.lineWidth=.8;ctx.stroke();ctx.restore();
}

function drawPieceOnCanvas(piece){
  const px=piece.x,py=piece.y,m=TAB_SIZE*cellW*1.3;
  const isDrag=dragging&&dragging.id===piece.id,isSel=selectedPieceId===piece.id;
  const lift=isDrag?1.04:1,sDist=isDrag?6:3;
  ctx.save();ctx.translate(px+cellW/2,py+cellH/2);ctx.scale(lift,lift);ctx.rotate(piece.rotation*Math.PI/180);ctx.translate(-(px+cellW/2),-(py+cellH/2));
  ctx.translate(sDist,sDist+1);createPiecePath(ctx,piece.row,piece.col,px,py,cellW,cellH);
  ctx.fillStyle=isDrag?'rgba(40,10,50,0.3)':'rgba(40,10,50,0.18)';ctx.filter=`blur(${isDrag?6:3}px)`;ctx.fill();ctx.filter='none';ctx.restore();
  ctx.save();ctx.translate(px+cellW/2,py+cellH/2);ctx.scale(lift,lift);ctx.rotate(piece.rotation*Math.PI/180);ctx.translate(-(px+cellW/2),-(py+cellH/2));
  ctx.translate(1,2);createPiecePath(ctx,piece.row,piece.col,px,py,cellW,cellH);ctx.fillStyle='rgba(80,30,60,0.25)';ctx.fill();ctx.restore();
  ctx.save();ctx.translate(px+cellW/2,py+cellH/2);ctx.scale(lift,lift);ctx.rotate(piece.rotation*Math.PI/180);ctx.translate(-(px+cellW/2),-(py+cellH/2));
  createPiecePath(ctx,piece.row,piece.col,px,py,cellW,cellH);ctx.clip();
  const sx=piece.col*(img.width/GRID),sy=piece.row*(img.height/GRID),sw=img.width/GRID,sh=img.height/GRID,sm=TAB_SIZE*Math.max(sw,sh)*1.3;
  ctx.drawImage(img,sx-sm,sy-sm,sw+sm*2,sh+sm*2,px-m,py-m,cellW+m*2,cellH+m*2);
  createPiecePath(ctx,piece.row,piece.col,px,py,cellW,cellH);
  const bevel=ctx.createLinearGradient(px-m,py-m,px+cellW+m,py+cellH+m);
  bevel.addColorStop(0,'rgba(255,255,255,0.22)');bevel.addColorStop(.25,'rgba(255,255,255,0.06)');bevel.addColorStop(.55,'transparent');bevel.addColorStop(.85,'rgba(40,10,50,0.08)');bevel.addColorStop(1,'rgba(40,10,50,0.15)');
  ctx.fillStyle=bevel;ctx.fill();
  const spec=ctx.createRadialGradient(px+cellW*.25,py+cellH*.2,0,px+cellW*.25,py+cellH*.2,cellW*.45);
  spec.addColorStop(0,'rgba(255,255,255,0.15)');spec.addColorStop(.5,'rgba(255,255,255,0.03)');spec.addColorStop(1,'transparent');ctx.fillStyle=spec;ctx.fill();ctx.restore();
  ctx.save();ctx.translate(px+cellW/2,py+cellH/2);ctx.scale(lift,lift);ctx.rotate(piece.rotation*Math.PI/180);ctx.translate(-(px+cellW/2),-(py+cellH/2));
  createPiecePath(ctx,piece.row,piece.col,px,py,cellW,cellH);ctx.strokeStyle='rgba(60,20,50,0.2)';ctx.lineWidth=2;ctx.stroke();
  createPiecePath(ctx,piece.row,piece.col,px,py,cellW,cellH);ctx.strokeStyle=isSel?'#E8679A':'rgba(255,255,255,0.15)';ctx.lineWidth=isSel?2:.8;ctx.stroke();ctx.restore();
}
...
function hitTest(mx,my){
  ...
  for(const p of c){const m=TAB_SIZE*cellW*1.3;if(mx>=p.x-m&&mx<=p.x+cellW+m&&my>=p.y-m&&my<=p.y+cellH+m)return p;}
  return null;
}
...
// Appelé à chaque placement — les milestones popent immédiatement
...
function scheduleNextEncouragement(){
  const delay=(15+Math.random()*10)*1000; // 15–25s
